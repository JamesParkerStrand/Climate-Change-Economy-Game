		...
		while (true) {
			...
			for (int i = 0; i > turnCount; i++) {
				...
				if (action.equals("start")) {
					...
					// now we find the difference of change to see how people are doing in terms of their housing
					int ChangeOfPopulationInHomes = amountOfPeopleInsideHomesAfter - amountOfPeopleInsideHomesBefore;

					// change the fact that the amount of people inside homes, will essentially make this go down. if not this goes up.
					...
					// if amount of people homeless is a negative number, just make it zero, it means we have no one homeless.
					...
					System.out.println("population is: " + (amountOfPeopleInsideHomesAfter + amountOfPeopleNeededForHomes) );
